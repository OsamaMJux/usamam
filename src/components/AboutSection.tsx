...
const AboutSection = () => {
  const values = [{
    ...
    description: "Build leverage through smart systems and automation, not just hard work. Work smarter, scale faster."
  }, {
    icon: Target,
    title: "Clarity Over Complexity",
    description: "Cut through trends and noise. Focus on what actually works with clear, actionable frameworks."
  }, {
    icon: Lightbulb,
    title: "Execution Over Ideas",
    description: "Ideas are cheap, execution is everything. I help turn visions into tangible, measurable results."
  }];
  return <section id="about" className="py-24 md:py-32 bg-gradient-card">
      <div className="container mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-6">
            {values.map((value, index) => <RevealOnScroll key={index} direction="right" delay={index * 0.15}>
                <TiltCard rotationIntensity={5}>
                  <motion.div className="group p-6 rounded-2xl bg-secondary/50 border border-border hover:border-primary/50 transition-all duration-300" whileHover={{
                scale: 1.02
              }}>
                    <div className="flex items-start gap-4">
                      <motion.div className="p-3 rounded-xl bg-primary/10 text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-colors" whileHover={{
                    rotate: [0, -10, 10, 0]
                  }} transition={{
                    duration: 0.5
                  }}>
                      ...
                      <div>
                        <h3 className="text-lg text-foreground mb-2 font-normal">
                          {value.title}
                        </h3>
