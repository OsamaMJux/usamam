...
const TestimonialsSection = () => {
  ...
  return <section id="testimonials" className="py-24 md:py-32 bg-background overflow-hidden">
      <div className="container mx-auto px-6">
          ...
          <div className="relative max-w-4xl mx-auto mb-16">
            <AnimatePresence mode="wait">
              ...
              duration: 0.5
                ...
                <p className="text-2xl md:text-3xl italic text-foreground leading-relaxed mb-8 font-sans">
            ...
            <div className="flex justify-center gap-3 mt-8">
              {testimonials.map((_, index) => <motion.button key={index} onClick={() => setActiveIndex(index)} className={`h-2 rounded-full transition-all duration-300 ${index === activeIndex ? "bg-primary w-8" : "bg-muted w-2"}`} whileHover={{
              scale: 1.2
            }} whileTap={{
              scale: 0.9
            }} />)}
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => <RevealOnScroll key={index} direction="up" delay={index * 0.15}>
              <TiltCard rotationIntensity={6}>
                <motion.div className="group p-8 rounded-2xl bg-secondary/50 border border-border hover:border-primary/50 transition-all duration-500 h-full" whileHover={{
              y: -5
            }}>
                  <motion.div initial={{
                rotate: 0
                ...
                duration: 0.5
                  ...
                  <p className="text-foreground leading-relaxed mb-8 italic font-sans text-sm font-extralight">
                    "{testimonial.quote}"
                  </p>
                  <div className="flex items-center gap-4">
                    <motion.div className="w-12 h-12 rounded-full bg-gradient-red flex items-center justify-center text-primary-foreground font-semibold" whileHover={{
                  scale: 1.1,
                  rotate: 360
                }} transition={{
                  duration: 0.5
        ...
        <RevealOnScroll delay={0.3}>
          <div className="mt-20 pt-12 border-t border-border">
            ...
            <div className="flex flex-wrap justify-center items-center gap-12">
              {companies.map((company, index) => <motion.span key={index} className="text-2xl font-serif font-bold text-muted-foreground/30 hover:text-primary transition-colors cursor-default" initial={{
              opacity: 0,
              y: 20
            }} whileInView={{
              opacity: 1,
