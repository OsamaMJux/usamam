...
const PHONE_NUMBER = "923214472719"; // WhatsApp number, no +
...
const buildLink = (message?: string) => {
  ...
  return `${base}?text=${encodeURIComponent(`Hi Usama,\n\n${message}`)}`;
};
...
const WhatsAppSticky = () => {
  ...
  return (
    <>
      {/* Floating Button — anchor so it works inside sandboxed iframes too */}
      <a
        href={buildLink()}
        target="_blank"
        rel="noopener noreferrer"
        onClick={(e) => {
          // Toggle the question popup instead of navigating when not yet open
          if (!open) {
            e.preventDefault();
            setOpen(true);
          }
        }}
        className="fixed bottom-6 right-6 z-[9999] flex items-center gap-2 rounded-full bg-neutral-900 px-4 py-2 text-sm font-medium text-white shadow-xl transition-all hover:-translate-y-0.5 hover:bg-[#25D366] no-underline"
      >
          ...
          <div className="space-y-2">
            {questions.map((q, i) => (
              <a
                ...
                className="block w-full rounded-xl bg-neutral-100 px-3 py-2 text-left text-sm text-neutral-700 transition hover:bg-neutral-200 no-underline"
              >
                {q}
              </a>
            ))}
